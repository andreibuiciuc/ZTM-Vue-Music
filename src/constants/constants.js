export default {
  ALERT_MESSAGES: {
    REGISTER_WAIT: 'Please wait! Your account is being created.',
    REGISTER_SUCCESS: 'Success! Your account has been created.',
    LOGIN_WAIT: 'Please wait! We are loggin you in.',
    LOGIN_SUCCESS: 'Success! You are now logged in.',
    LOGIN_ERROR: 'Invalid login details',
    ERROR: 'An unexpected error occurred. Please try again later.',
    UPDATE_WAIT: 'Please wait! Updating composition info',
    UPDATE_SUCCESS: 'Success! Your composition has been updated',
    COMMENT_SUBMISSION_WAIT: 'Please wait! Your comment is being submitted',
    COMMENT_SUBMISSION_SUCCESS: 'Comment added!'
  },
  COLOR_VARIANTS: {
    BLUE: 'bg-blue-500',
    GREEN: 'bg-green-500',
    RED: 'bg-red-500'
  }
};
