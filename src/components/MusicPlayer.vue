  ...
  <div class="fixed bottom-0 left-0 bg-white p-5 pb-4 text-left align-top w-full h-16">
    <div class="relative">
      ...
      <div class="float-left w-7 h-7 leading-3">
        <button type="button" @click.prevent="toggleAudio">
          <i class="fa text-gray-500 text-xl" 
            :class="{ 'fa-play': !isSongPlaying, 'fa-pause': isSongPlaying }"></i>
        </button>
      </div>
      <!-- Current Position -->
      ...
      <div class="float-left w-7 h-7 leading-3 ml-7 mt-2 player-scrub">
        <div v-if="currentSong.modifiedName" class="absolute left-0 right-0 text-lg text-center mx-auto player-song-info">
        ...
        <div class="block w-full h-2 rounded m-1 mt-2 bg-gray-200 relative cursor-pointer"
          ...
          <!-- Player Ball -->
          <span class="absolute top-neg-8 text-gray-800 text-lg" :style="{ left: progressPosition }">
            <i class="fas fa-circle"></i>
          </span>
          <!-- Player Progress Bar-->
          <span class="block h-2 rounded bg-gradient-to-r from-green-500 to-green-400"
            :style="{ width: progressPosition }"></span>
        </div>
      </div>
      <!-- Duration -->
...
export default {
  name: 'MusicPlayer',
  computed: {
    ...mapState(usePlayerStore, ['currentSong', 'isSongPlaying', 'seek', 'duration', 'progressPosition'])
  },
  methods: {
